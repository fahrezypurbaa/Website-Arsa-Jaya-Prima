        ...
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active" aria-current="page">
              <i class="fa-solid fa-pencil me-1"></i> Edit Artikel
            </li>
          </ol>
        </nav>
      </div>
        ...
        <div class="col-md-10">
            ...
            <div class="mb-3">
                <label for="tags" class="form-label">
                    Tags <span class="text-danger">*pisahkan dengan koma (,)</span>
                </label>
                {{-- {{ old('tags', $post->tags()->all()->name) }} --}}
                <input type="text" class="form-control @error('tags') is-invalid @enderror" id="form-tags" name="tags" autofocus value="{{ old('tags', implode(', ', $post->tags->pluck('name')->toArray() ?? [] )) }}">
            </div>
            <h4>Kebutuhan SEO</h4>
            <div class="mb-3">
                ...
                >
                <textarea id="meta_desc" name="meta_desc" class="form-control meta_desc" style="height: 100px">{{ old('meta_desc', $post->meta_desc) }}</textarea>
                @error('meta_desc')
                    <p class="text-danger">{{ $message }}</p>
                @enderror
            </div>
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('keywords') is-invalid @enderror" id="keywords" name="keywords" required autofocus value="{{ old('keywords', $post->keywords) }}">
                @error('keywords')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
