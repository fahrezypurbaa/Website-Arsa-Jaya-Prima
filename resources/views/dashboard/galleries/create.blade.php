    ...
    <div class="places-section section-padding px-3 py-2 bg-white rounded">
        ...
        <div class="row">
            <div class="col-md-10">
                ...
                <div class="mb-3">
                    <label for="gallery_categories" class="form-label"
                    >Kategori <span class="text-danger">*</span></label
                    >
                    <select class="form-select" name="gallery_category_id">
                        @foreach ($gallery_categories as $gallery_category)
                            @if (old('gallery_category_id') == $gallery_category->id)
                                <option value="{{ $gallery_category->id }}" selected>{{ $gallery_category->id }}</option>
                            @else
                ...
                <div class="mb-3">
                    ...
                    <input class="form-control @error('photos') is-invalid @enderror" type="file" id="photos" name="photos[]" accept="image/*" multiple onchange="previewImage()">
                        @error('photos')
                            <div class="invalid-feedback">
                                {{ $message }}
                            </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('schedule') is-invalid @enderror" id="schedule" name="schedule" required autofocus value="{{ old('schedule') }}">
                    @error('schedule')
                        <div class="invalid-feedback">
                            {{ $message }}
                        </div>
